...
- **`generate_wdio_config`** — Generate a minimal `wdio.conf.ts` for Appium + React Native, including the install command for all required packages. Optional `udid`, `deviceName`, and `platformVersion` values select a device without embedding fixed simulator defaults. `noReset` defaults to `true` to preserve the installed app and its data during replay.
